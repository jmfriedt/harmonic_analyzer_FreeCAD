...
# import create_triangle
# ensuite pour recharger en cours de developpment : 
# import importlib
# importlib.reload(create_triangle)
...
def update_harmonic():
  ...
  for t in harm:
   ...
   y1 = roue[k].Radius/(2*k+1)*(sin(pow(-1,k)*radians(angle)))
   rouepignon[k].Placement = FreeCAD.Placement( roueinit[k].Placement.Base , FreeCAD.Rotation ( FreeCAD.Vector( 0,0,1), pow(-1,k)*angle) )
...
for k in range(nbharmonics):
 ...
 pignon.Height=1
 pignon.Radius=0.5
 pignon.Placement.Base.x=tmproue.Radius.Value/(2*k+1) # -1
...
for r in roue:
 if k==0:
  ...
  rouepignon[k].Placement.Base.x=position+r.Radius
  position=position+2*r.Radius
 k+=1
